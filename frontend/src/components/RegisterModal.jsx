import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth, BACKEND_URL } from '../context/AuthContext';
import { X } from 'lucide-react';

export default function RegisterModal({ isOpen, onClose }) {
  if (!isOpen) return null;

  const { loginWithGoogleOAuth } = useAuth();
  const navigate = useNavigate();
  
  const [clientId, setClientId] = useState('336110271991-cmeqben5o90fo0ff3dh4gd0uajcsv49s.apps.googleusercontent.com');
  const [error, setError] = useState('');
  const [agreed, setAgreed] = useState(false);

  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/api/auth/config`);
        if (res.ok) {
          const data = await res.json();
          if (data.clientId) {
            setClientId(data.clientId);
          }
        }
      } catch (err) {
        console.error('Failed to load Google Client Config:', err);
      }
    };
    fetchConfig();
  }, []);

  useEffect(() => {
    if (!clientId || !isOpen || !agreed) return;

    if (window.google?.accounts?.id) {
      try {
        window.google.accounts.id.initialize({
          client_id: clientId,
          callback: async (response) => {
            try {
              await loginWithGoogleOAuth(response.credential, true);
              onClose();
              navigate('/');
            } catch (err) {
              console.error('Google OAuth Sign Up failed:', err);
              setError(err.message || 'Sign up failed');
            }
          }
        });

        setTimeout(() => {
          const btnDiv = document.getElementById('googleSignUpDiv');
          if (btnDiv) {
            btnDiv.innerHTML = '';
            window.google.accounts.id.renderButton(
              btnDiv,
              { theme: 'outline', size: 'large', width: '380', text: 'continue_with' }
            );
          }
        }, 100);
      } catch (err) {
        console.error('Failed to initialize Google GSI for register:', err);
      }
    }
  }, [clientId, isOpen, agreed]);

  useEffect(() => {
    document.body.style.overflow = 'hidden';
    return () => {
      document.body.style.overflow = 'unset';
    };
  }, []);

  const handleGoToLogin = () => {
    onClose();
    navigate('/login');
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/45 backdrop-blur-[3px] animate-fade-in">
      <div 
        className="bg-white rounded-xl shadow-[0_24px_50px_-12px_rgba(0,0,0,0.15)] w-full max-w-[480px] flex flex-col relative overflow-hidden border border-slate-200 p-10 pt-12 pb-10 font-sans"
        onClick={e => e.stopPropagation()}
      >
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-slate-650 hover:bg-slate-50 transition cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>

        {/* Header Block */}
        <div className="flex flex-col text-left w-full mb-8">
          <h2 className="text-[28px] font-bold text-slate-900 tracking-tight leading-tight">
            Join us<br/>
            Create an account
          </h2>
          <p className="text-[14px] text-slate-600 mt-3">
            Be part of the premier student developer community
          </p>
        </div>

        {error && (
          <div className="p-3 mb-5 rounded bg-rose-50 border border-rose-100 text-rose-600 text-sm flex items-center gap-2">
            {error}
          </div>
        )}

        {/* Terms Checkbox */}
        <div className="flex items-start gap-3 mb-8 px-1">
          <input 
            type="checkbox" 
            id="terms" 
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
          />
          <label htmlFor="terms" className="text-[13px] text-slate-600 leading-relaxed cursor-pointer select-none">
            I agree to the <span className="text-blue-600 hover:underline">Terms of Service</span> and <span className="text-blue-600 hover:underline">Privacy Policy</span>.
          </label>
        </div>

        <div className="w-full flex flex-col items-center justify-center min-h-[44px]">
          {!agreed ? (
             <div className="w-[380px] h-[40px] border border-slate-200 rounded flex items-center justify-center bg-slate-50 text-slate-400 text-[13px] font-medium cursor-not-allowed">
               Check the box above to continue
             </div>
          ) : (
            <div id="googleSignUpDiv"></div>
          )}
        </div>

        {/* Go to login bottom link */}
        <div className="text-center mt-8 pt-6 border-t border-slate-100">
          <p className="text-[14px] text-slate-600">
            Already have an account?{' '}
            <button 
              type="button"
              onClick={handleGoToLogin} 
              className="text-blue-600 hover:underline font-medium bg-transparent border-none cursor-pointer p-0"
            >
              Log in
            </button>
          </p>
        </div>

      </div>
    </div>
  );
}
